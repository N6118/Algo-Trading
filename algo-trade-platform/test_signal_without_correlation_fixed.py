#!/usr/bin/env python3
"""
Test Signal Scanner Without Correlation (Fixed Version)
...
def test_signal_without_correlation():
    ...
    try:
        ...
        # Temporarily disable correlation threshold by setting it to -1.0 (very low)
        logger.info("🔧 Temporarily disabling correlation threshold...")
        
        update_query = text("""
            UPDATE signal_entry_rules 
            SET correlation_threshold = -1.0 
            WHERE rule_type = 'Correlation'
        """)
        
        session.execute(update_query)
        session.commit()
        logger.info("✅ Correlation threshold temporarily set to -1.0")
...
def run_test_without_correlation():
    ...
    logger.info("🚀 SIGNAL SCANNER TEST WITHOUT CORRELATION (FIXED)")
    logger.info("=" * 70)
